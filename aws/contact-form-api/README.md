...
```bash
contact-form-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
contact-form-api$ sam local start-api
contact-form-api$ curl http://localhost:3000/
```
...
```bash
contact-form-api$ sam logs -n HelloWorldFunction --stack-name contact-form-api --tail
```
...
```bash
contact-form-api$ cd hello-world
...
```bash
sam delete --stack-name contact-form-api
```
